...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/college/login`, formData, { withCredentials: true });
    ...
    <div className="min-h-screen bg-[#F5EFFF] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="bg-white shadow-lg rounded-lg overflow-hidden">
          <h2 className="text-2xl font-bold text-center text-white bg-[#4A0E5C] py-4">College Login</h2>
          <div className="p-6 space-y-4">
            ...
            <div className="text-right">
              <a href="/forgot-password" className="text-sm text-[#A594F9] hover:underline">
                Forgot Password?
              </a>
            </div>
            <button
            ...
            <div className="text-center space-y-2">
              <p className="text-sm text-[#4A0E5C]">
                ...
                <a href="/registerpage" className="text-[#A594F9] hover:underline">Register</a>
              </p>
              <p className="text-sm text-[#4A0E5C]">
                <a href="/teacherlogin" className="text-[#A594F9] hover:underline">Teacher Login</a>
              </p>
            </div>
          </div>
        </form>
